import React from "react";
import StyledSanFran from "./StyledSanFran";
import LogoBanner from "../../../components/shared/LogoBanner/LogoBanner";

function SanFranPolicyStatic() {
  return (
    <>
      <StyledSanFran>
        <div className="top-left-container">
          <LogoBanner title="San Francisco" />
          <p className="in-works">In the Works</p>
        </div>
        <div className="wrapper">
          <div className="top-right-container">
            <div className="input-container">
              <p className="enter-location">Or enter your location:</p>
              <input
                className="top-right-input"
                type="text"
                placeholder="Zipcode or City, State"
              />
            </div>
          </div>
          <div className="middle-wrapper">
            <div className="middle-container">
              <div className="content-container">
                <img
                  className="content-image"
                  src="https://i.imgur.com/TKt1r4C.png"
                  alt="Commuter Benefits Ordinance"
                />
                <img
                  className="mobile-image"
                  src="https://i.imgur.com/StT7g4I.png"
                  alt="person holding a bag"
                />
                <div className="total-text-container">
                  <div className="title-text-container">
                    <h1 className="box-text-title">
                      Commuter Benefits Ordinance
                    </h1>
                  </div>
                  <div className="box-text-container">
                    <p className="box-text">
                      Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                      sed diam nonummy nibh euismod tincidunt ut laoreet dolore
                      magna aliquam erat volutpat. Ut wisi enim ad minim veniam,
                      quis nostrud exerci tation ullamcorper suscipit lobortis
                      nisl ut aliquip ex ea commodo consequat. Duis autem vel
                      eum iriure dolor in hendrerit in vulputate velit esse
                      molestie consequat, vel illum dolore eu feugiat nulla
                      facilisis at vero eros et accumsan et iusto odio dignissim
                      qui blandit praesent luptatum zzril delenit augue duis
                      dolore te feugait nulla facilisi. Lorem ipsum dolor sit
                      amet, cons ectetuer adipiscing elit, sed diam nonummy nibh
                      euismod
                    </p>
                    <p className="box-text-mobile">
                      Requires that all covered employers offer to their covered
                      employees at least one commuter benefits option.
                    </p>
                    <span className="box-text-mobile">Read more ></span>
                  </div>
                </div>
              </div>
              <div className="content-container">
                <img
                  className="content-image"
                  src="https://i.imgur.com/1gxvw3j.png"
                  alt="Tenant Bicycle Parking"
                />
                <img
                  className="mobile-image"
                  src="https://i.imgur.com/IkVQtme.png"
                  alt="a bicycle"
                />
                <div className="total-text-container">
                  <div className="title-text-container">
                    <h1 className="box-text-title">Tenant Bicycle Parking </h1>
                  </div>
                  <div className="box-text-container">
                    <p className="box-text">
                      Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                      sed diam nonummy nibh euismod tincidunt ut laoreet dolore
                      magna aliquam erat volutpat. Ut wisi enim ad minim veniam,
                      quis nostrud exerci tation ullamcorper suscipit lobortis
                      nisl ut aliquip ex ea commodo consequat. Duis autem vel
                      eum iriure dolor in hendrerit in vulputate velit esse
                      molestie consequat, vel illum dolore eu feugiat nulla
                      facilisis at vero eros et accumsan et iusto odio dignissim
                      qui blandit praesent luptatum zzril delenit augue duis
                      dolore te feugait nulla facilisi. Lorem ipsum dolor sit
                      amet, cons ectetuer adipiscing elit, sed diam nonummy nibh
                      euismod
                    </p>
                    <p className="box-text-mobile">
                      Owners of commercial buildings must either provide secure
                      bicycle parking or allow tenants to bringtheir bicycles
                      into the building.
                    </p>
                    <span className="box-text-mobile">Read more ></span>
                  </div>
                </div>
              </div>

              <div className="content-container">
                <img
                  className="content-image"
                  src="https://i.imgur.com/IbItVIU.png"
                  alt="Congestion Pricing"
                />
                <img
                  className="mobile-image"
                  src="https://i.imgur.com/6OdfDUQ.png"
                  alt="Congestion Pricing"
                />
                <div className="total-text-container">
                  <div className="title-text-container">
                    <h1 className="box-text-title">Congestion Pricing </h1>
                  </div>
                  <div className="box-text-container">
                    <p className="box-text">
                      Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                      sed diam nonummy nibh euismod tincidunt ut laoreet dolore
                      magna aliquam erat volutpat. Ut wisi enim ad minim veniam,
                      quis nostrud exerci tation ullamcorper suscipit lobortis
                      nisl ut aliquip ex ea commodo consequat. Duis autem vel
                      eum iriure dolor in hendrerit in vulputate velit esse
                      molestie consequat, vel illum dolore eu feugiat nulla
                      facilisis at vero eros et accumsan et iusto odio dignissim
                      qui blandit praesent luptatum zzril delenit augue duis
                      dolore te feugait nulla facilisi. Lorem ipsum dolor sit
                      amet, cons ectetuer adipiscing elit, sed diam nonummy nibh
                      euismod
                    </p>
                    <p className="box-text-mobile">
                      Supports the evaluation of congestion pricing to develop
                      environmentally beneficial congestion management programs.
                    </p>
                    <span className="box-text-mobile">Read more ></span>
                  </div>
                </div>
              </div>
              <p className="get-involved">Get Involved</p>
              <div className="content-container-mobile">
                <img
                  className="content-image"
                  src="https://i.imgur.com/bgRDJbf.jpg"
                  alt="Divestment from Petroleum"
                />
                <img
                  className="mobile-image"
                  src="https://i.imgur.com/nnHUJgE.png"
                  alt="Divestment from Petroleum"
                />
                <div className="total-text-container">
                  <div className="title-text-container">
                    <h1 className="box-text-title">
                      Divestment from Petroleum
                    </h1>
                  </div>
                  <div className="box-text-container">
                    <p className="box-text">
                      Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                      sed diam nonummy nibh euismod tincidunt ut laoreet dolore
                      magna aliquam erat volutpat. Ut wisi enim ad minim veniam,
                      quis nostrud exerci tation ullamcorper suscipit lobortis
                      nisl ut aliquip ex ea commodo consequat. Duis autem vel
                      eum iriure dolor in hendrerit in vulputate velit esse
                      molestie consequat, vel illum dolore eu feugiat nulla
                      facilisis at vero eros et accumsan et iusto odio dignissim
                      qui blandit praesent luptatum zzril delenit augue duis
                      dolore te feugait nulla facilisi. Lorem ipsum dolor sit
                      amet, cons ectetuer adipiscing elit, sed diam nonummy nibh
                      euismod
                    </p>
                    <p className="box-text-mobile">
                      Urge San Francisco's Retirement Board to stop supporting
                      Occidental Petroleum.
                    </p>
                    <span className="box-text-mobile">Read more ></span>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </StyledSanFran>
    </>
  );
}

export default SanFranPolicyStatic;
